...
This client uses [Click](https://click.palletsprojects.com/en/8.1.x/#documentation) under the hood to make it easy to
...
Try to avoid the use of positional arguments and use flags instead. Although using flags requires the user to write
more, it is actually better because it does not force the user to memorize the order of the arguments. Additionally, by
using meaningful names for each flag, it is easier for the user.
...
Each test will start with test_ then will be followed by the name of the function to test and then a short description.
You have to provide a docstring to the test linking the function that is being tested, you can provide any context that
you consider relevant.

Example:

```python
def test_extract_entities_failed_on_extractions():
  """
