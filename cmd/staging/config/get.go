...
func getCommandExecute(cmd *cobra.Command, d cli.Discovery) error {
	...
	return d.PrintStagingConfigToUser(profile, sensitive, true)
}
...
func NewGetCommand(d cli.Discovery) *cobra.Command {
	get := &cobra.Command{
		Use:   "get",
		Short: "Print Discovery Staging's configuration",
		Long:  "get is the command used to obtain Discovery Staging's configuration for a given profile. If the API keys are sensitive, the `sensitive` flag can be set to true in order to obfuscate them before printing them out. If a configuration property was not set, it is not displayed.",
