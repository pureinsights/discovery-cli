...
func TestNewStoreCommand(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			{
			"name": "my-seed-schedule-2",
			"expression": "0 1 * * *",
			"properties": {"some-property": "b"},
			"seed": "ac7c5765-bef6-42cc-b519-c75df51ebf3b",
			"scanType": "INCREMENTAL",
			"id": "9ababe08-0b74-4672-bb7c-e7a8227d6d4d"
			},
			{
			"name": "my-seed-schedule-3",
			"expression": "0 2 * * *",
			"properties": {"some-property": "c"},
			"seed": "ac7c5765-bef6-42cc-b519-c75df51ebf3b",
			"scanType": "FULL",
			...
			responses: map[string]testutils.MockResponse{
				...
				"PUT:/v2/seed/schedule/9ababe08-0b74-4672-bb7c-e7a8227d6d4d": {
					...
					Assertions: func(t *testing.T, r *http.Request) {
						...
						assert.Equal(t, "/v2/seed/schedule/9ababe08-0b74-4672-bb7c-e7a8227d6d4d", r.URL.Path)
						assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
					},
				},
				"PUT:/v2/seed/schedule/9ababe08-0b74-4672-bb7c-e7a8227d6dad": {
					StatusCode: http.StatusOK,
					Body: `{
					"name": "my-seed-schedule-3",
					"expression": "0 2 * * *",
					...
					Assertions: func(t *testing.T, r *http.Request) {
						assert.Equal(t, http.MethodPut, r.Method)
						assert.Equal(t, "/v2/seed/schedule/9ababe08-0b74-4672-bb7c-e7a8227d6dad", r.URL.Path)
						assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
					},
				},
			...
			"pageNumber": 0
				...
				"GET:/v2/seed/schedule/9ababe08-0b74-4672-bb7c-e7a8227d6dad": {
					StatusCode: http.StatusOK,
					Body: `{
				"name": "my-seed-schedule-3",
				"expression": "0 2 * * *",
...
func TestNewStoreCommand_MultipleFiles(t *testing.T) {
	responses := map[string]testutils.MockResponse{
		...
		"PUT:/v2/seed/schedule/2fba75cd-e5dc-454f-b028-a505f0c690b2": {
			StatusCode: http.StatusOK,
			Body: `{
				"name": "my-seed-schedule-4",
				"expression": "0 3 * * *",
				"properties": {"some-property": "d"},
				"seed": "ac7c5765-bef6-42cc-b519-c75df51ebf3b",
				"scanType": "INCREMENTAL",
			...
			Assertions: func(t *testing.T, r *http.Request) {
				assert.Equal(t, http.MethodPut, r.Method)
				assert.Equal(t, "/v2/seed/schedule/2fba75cd-e5dc-454f-b028-a505f0c690b2", r.URL.Path)
				assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
			},
		},
		"PUT:/v2/seed/schedule/77cd87e5-5c29-4960-9058-4a2eda0c513f": {
			StatusCode: http.StatusOK,
			Body: `{
				"name": "my-seed-schedule-5",
				"expression": "0 4 * * *",
				"properties": {"some-property": "e"},
				"seed": "ac7c5765-bef6-42cc-b519-c75df51ebf3b",
				"scanType": "FULL",
			...
			Assertions: func(t *testing.T, r *http.Request) {
				assert.Equal(t, http.MethodPut, r.Method)
				assert.Equal(t, "/v2/seed/schedule/77cd87e5-5c29-4960-9058-4a2eda0c513f", r.URL.Path)
				assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
			},
		},
	}
	...
	vpr.Set("output", "json")
	vpr.Set("default.ingestion_url", srv.URL)
	vpr.Set("default.ingestion_key", "apiKey123")
