...
func getCommandExecute(cmd *cobra.Command, d cli.Discovery) error {
	...
	return d.PrintIngestionConfigToUser(profile, sensitive, true)
}
...
func NewGetCommand(d cli.Discovery) *cobra.Command {
	get := &cobra.Command{
		...
		Long:  "get is the command used to obtain Discovery Ingestion's configuration for a given profile. If the API keys are sensitive, the `sensitive` flag can be set to true in order to obfuscate them before printing them out. If a configuration property was not set, it is not displayed.",
