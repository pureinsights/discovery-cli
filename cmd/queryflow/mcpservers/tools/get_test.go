...
func TestNewGetCommand(t *testing.T) {
	tests := []struct {
		...
		// Working case
		{
			...
			"pageNumber": 0
				...
				"GET:/v2/entrypoint/mcp-server/4957145b-6192-4862-a5da-e97853974e9f/tool/3b32e410-2f33-412d-9fb8-17970131921c": {
					StatusCode: http.StatusOK,
					Body: `{
					"type": "mongo",
					"name": "my-mcp-tool",
					...
					Assertions: func(t *testing.T, r *http.Request) {
						assert.Equal(t, http.MethodGet, r.Method)
						assert.Equal(t, "/v2/entrypoint/mcp-server/4957145b-6192-4862-a5da-e97853974e9f/tool/3b32e410-2f33-412d-9fb8-17970131921c", r.URL.Path)
					},
				},
			},
			err: nil,
		},
		{
			...
			"pageNumber": 0
				...
				"GET:/v2/entrypoint/mcp-server/4957145b-6192-4862-a5da-e97853974e9f/tool": {
					StatusCode: http.StatusOK,
					Body: `{
			"content": [
				{
		...
		},
		{
			...
			args:      []string{"my-mcp-server", "--filter", "label=A:A"},
	...
	"messages": [
		...
		{
			name:      "GetCommand receives an MCP server that does not exist.",
			args:      []string{"my-mcp-server"},
			url:       true,
			apiKey:    "apiKey123",
			outGolden: "NewGetCommand_Out_MCPServerDoesNotExist",
			errGolden: "NewGetCommand_Err_MCPServerDoesNotExist",
			outBytes:  []byte(nil),
			errBytes:  testutils.Read(t, "NewGetCommand_Err_MCPServerDoesNotExist"),
			responses: map[string]testutils.MockResponse{
				"POST:/v2/entrypoint/mcp-server/search": {
					StatusCode:  http.StatusNoContent,
					Body:        ``,
	...
	"messages": [
		...
		},
		{
			...
			args:      []string{"my-mcp-server", "3b32e410-2F33-412d-9fb8-17970131921c"},
		...
		},
		{
			...
			args:      []string{"my-mcp-server", "--filter", "label=A"},
			url:       true,
			apiKey:    "apiKey123",
	...
	"timestamp": "2025-09-30T15:38:42.885125200Z"
		...
		},
		{
			...
			args:      []string{"my-mcp-server", "--filter", "gte=field:1"},
			url:       true,
			apiKey:    "apiKey123",
...
// TestNewGetCommand_NoMCPServer tests the NewGetCommand when the MCP server argument was not included.
func TestNewGetCommand_NoMCPServer(t *testing.T) {
	in := strings.NewReader("")
	out := &bytes.Buffer{}
	...
	testutils.CompareBytes(t, "NewGetCommand_Err_NoMCPServer", testutils.Read(t, "NewGetCommand_Err_NoMCPServer"), errBuf.Bytes())
}
