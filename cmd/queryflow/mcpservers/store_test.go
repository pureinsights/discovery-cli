	...
	tests := []struct {
		...
		},
		{
			...
			responses: map[string]testutils.MockResponse{
				...
				"PUT:/v2/entrypoint/mcp-server/9ababe08-0b74-4672-bb7c-e7a8227d6d4d": {
					...
					Assertions: func(t *testing.T, r *http.Request) {
						...
						assert.Equal(t, "/v2/entrypoint/mcp-server/9ababe08-0b74-4672-bb7c-e7a8227d6d4d", r.URL.Path)
						assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
					},
				},
				"PUT:/v2/entrypoint/mcp-server/9ababe08-0b74-4672-bb7c-e7a8227d6dad": {
					StatusCode: http.StatusOK,
					Body: `{
					"type": "openai",
					...
					Assertions: func(t *testing.T, r *http.Request) {
						assert.Equal(t, http.MethodPut, r.Method)
						assert.Equal(t, "/v2/entrypoint/mcp-server/9ababe08-0b74-4672-bb7c-e7a8227d6dad", r.URL.Path)
						assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
					},
				},
			...
			"pageNumber": 0
				...
				"GET:/v2/entrypoint/mcp-server/9ababe08-0b74-4672-bb7c-e7a8227d6dad": {
					StatusCode: http.StatusOK,
					Body: `{
				"type": "openai",
				"name": "OpenAI mcp-server 3",
...
func TestNewStoreCommand_MultipleFiles(t *testing.T) {
	responses := map[string]testutils.MockResponse{
		...
		"PUT:/v2/entrypoint/mcp-server/2fba75cd-e5dc-454f-b028-a505f0c690b2": {
			StatusCode: http.StatusOK,
			Body: `{
				"type": "mongo",
				"name": "MongoDB mcp-server 4",
				"labels": [],
				"active": true,
			...
			Assertions: func(t *testing.T, r *http.Request) {
				assert.Equal(t, http.MethodPut, r.Method)
				assert.Equal(t, "/v2/entrypoint/mcp-server/2fba75cd-e5dc-454f-b028-a505f0c690b2", r.URL.Path)
				assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
			},
		},
		"PUT:/v2/entrypoint/mcp-server/77cd87e5-5c29-4960-9058-4a2eda0c513f": {
			StatusCode: http.StatusOK,
			Body: `{
				"type": "openai",
				"name": "OpenAI mcp-server 5",
				"labels": [],
				"active": true,
			...
			Assertions: func(t *testing.T, r *http.Request) {
				assert.Equal(t, http.MethodPut, r.Method)
				assert.Equal(t, "/v2/entrypoint/mcp-server/77cd87e5-5c29-4960-9058-4a2eda0c513f", r.URL.Path)
				assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
			},
		},
	}
