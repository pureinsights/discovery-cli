...
const (
	...
	LongExport string = "export is the command used to backup Discovery %s's entities. With the --output-file flag, the user can send the specific file in which to save the configurations. If not, they will be saved in a zip file in the current directory."
	// LongImport is the message used in the Long field of the Import commands.
	LongImport string = "import is the command used to restore Discovery %s's entities. With the required argument, the user must send the specific file that has the entities' configuration. With the --on-conflict flag, the user can send the conflict resolution strategy in case there are duplicate entities."
)
