...
func NewDeleteCommand(d cli.Discovery) *cobra.Command {
	get := &cobra.Command{
		...
		Long:  fmt.Sprintf(cli.LongDeleteNoNames, "server", "Core"),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cli.SearchDeleteCommand(args[0], d, coreClient.Servers(), cli.GetCommandConfig(profile, vpr.GetString("output"), "Core", "core_url", "core_key"))
		},
