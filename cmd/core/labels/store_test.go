			...
			responses: map[string]testutils.MockResponse{
				"POST:/v2/label": {
					...
					Assertions: func(t *testing.T, r *http.Request) {
						...
						assert.Equal(t, "/v2/label", r.URL.Path)
					},
				},
			},
			err: nil,
			...
			responses: map[string]testutils.MockResponse{
				...
				"PUT:/v2/label/9ababe08-0b74-4672-bb7c-e7a8227d6d4d": {
					...
					Assertions: func(t *testing.T, r *http.Request) {
						...
						assert.Equal(t, "/v2/label/9ababe08-0b74-4672-bb7c-e7a8227d6d4d", r.URL.Path)
						assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
					},
				},
				"PUT:/v2/label/9ababe08-0b74-4672-bb7c-e7a8227d6dad": {
					...
					Assertions: func(t *testing.T, r *http.Request) {
						assert.Equal(t, http.MethodPut, r.Method)
						assert.Equal(t, "/v2/label/9ababe08-0b74-4672-bb7c-e7a8227d6dad", r.URL.Path)
						assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
					},
				},
			},
...
func TestNewStoreCommand_MultipleFiles(t *testing.T) {
	responses := map[string]testutils.MockResponse{
		...
		"PUT:/v2/label/2fba75cd-e5dc-454f-b028-a505f0c690b2": {
			StatusCode: http.StatusOK,
			Body: `{
				"type": "mongo",
				"name": "MongoDB label 4",
				"labels": [],
				"active": true,
			...
			Assertions: func(t *testing.T, r *http.Request) {
				assert.Equal(t, http.MethodPut, r.Method)
				assert.Equal(t, "/v2/label/2fba75cd-e5dc-454f-b028-a505f0c690b2", r.URL.Path)
				assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
			},
		},
		"PUT:/v2/label/77cd87e5-5c29-4960-9058-4a2eda0c513f": {
			StatusCode: http.StatusOK,
			Body: `{
				"type": "openai",
				"name": "OpenAI label 5",
			...
			Assertions: func(t *testing.T, r *http.Request) {
				assert.Equal(t, http.MethodPut, r.Method)
				assert.Equal(t, "/v2/label/77cd87e5-5c29-4960-9058-4a2eda0c513f", r.URL.Path)
				assert.Equal(t, "apiKey123", r.Header.Get("X-API-Key"))
			},
		},
	}
