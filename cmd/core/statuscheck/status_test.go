...
func TestNewStatusCommand_ProfileFlagAndUpProduct(t *testing.T) {
    ...
    "status": "DOWN"
		...
		},
		{
			...
			url:        true,
			apiKey:     "",
			err:        cli.NewErrorWithCause(cli.ErrorExitCode, discoveryPackage.Error{Status: http.StatusInternalServerError, Body: gjson.Parse("{\"error\": \"Internal server error\"}")}, "Could not check the status of Discovery Core."),
		},
		{
