...
func TestNewImportCommand_ProfileFlag(t *testing.T) {
	...
	tests := []struct {
		...
		// Working case
		{
			...
			file:       filepath.Join("testdata", "core-export.zip"),
			err:        nil,
		},
		...
		},
		{
			...
			err:       cli.NewErrorWithCause(cli.ErrorExitCode, fmt.Errorf("file does not exist: %s", filepath.Join("doesnotexist", "core-export.zip")), "Could not import entities"),
		},
		{
			name:       "Import fails",
			url:        true,
...
func TestNewImportCommand_NoProfileFlag(t *testing.T) {
	...
	getCmd.SetArgs([]string{"--file", "testdata/core-export.zip"})
