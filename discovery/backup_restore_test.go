...
func Test_backupRestore_Import(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			dir, err := os.MkdirTemp(os.TempDir(), "backupTest-*")
			if err != nil {
				fmt.Println(err)
				return
			}
			fileName, err := fileutils.CreateTemporaryFile(dir, "test-export.zip", string(bytes))
			if err != nil {
				fmt.Println(err)
				return
			}

			b := backupRestore{client: newClient(srv.URL+tc.subClientPath, "")}
