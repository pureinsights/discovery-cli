...
func Test_discovery_StartSeed(t *testing.T) {
	tests := []struct {
		...
		// Working case
		{
			...
			client:         new(mocks.WorkingSeedController),
			printer:        nil,
		...
		{
			name:           "GetByIdFails",
			client:         new(mocks.FailingSeedControllerGetEntityIdFails),
			printer:        nil,
			expectedOutput: "",
		...
		},
		{
			...
			client:         new(mocks.FailingSeedControllerStartFails),
			printer:        nil,
			expectedOutput: "",
...
func Test_discovery_HaltSeedExecution(t *testing.T) {
	tests := []struct {
		...
		// Working case
		{
			...
			client:         new(mocks.WorkingSeedExecutionController),
			printer:        nil,
			expectedOutput: "{\n  \"acknowledged\": true\n}\n",
			err:            nil,
		},
		...
		{
			name:           "Halt fails seed not found",
			client:         new(mocks.FailingSeedExecutionControllerHaltFails),
			printer:        nil,
			expectedOutput: "",
			err: NewErrorWithCause(ErrorExitCode, discoveryPackage.Error{Status: http.StatusConflict, Body: gjson.Parse(`{
			"status": 409,
...
func TestAppendSeedRecord(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			client: new(mocks.WorkingRecordGetter),
  ...
  "status": "SUCCESS"
		...
		},
		{
			...
			client:         new(mocks.FailingRecordGetter),
			id:             "A3HTDEgCa65BFZsac9TInFisvloRlL3M50ijCWNCKx0=",
			expectedRecord: "",
...
{"timestamp":"2025-09-05T20:13:26.602Z","status":"RUNNING","stages":["BEFORE_HOOKS","INGEST"]}
		...
		},
		{
			...
			client: new(mocks.WorkingSeedExecutionGetter),
			summarizers: map[string]Summarizer{
				"records": new(mocks.NoContentRecordSummarizer),
				"jobs":    new(mocks.WorkingJobSummarizer),
			},
			expectedFields: map[string]string{
				"audit": `[
...
{"timestamp":"2025-09-05T20:13:26.602Z","status":"RUNNING","stages":["BEFORE_HOOKS","INGEST"]}
		...
		// Error case
		{
			...
			client: new(mocks.FailingSeedExecutionGetterAuditFails),
			summarizers: map[string]Summarizer{
				"records": new(mocks.NoContentRecordSummarizer),
				"jobs":    new(mocks.WorkingJobSummarizer),
			},
  ...
  "timestamp": "2025-11-17T19:32:01.555127800Z"
		...
		},
		{
			...
			summarizers: map[string]Summarizer{
				"records": new(mocks.WorkingRecordSummarizer),
				"jobs":    new(mocks.FailingJobSummarizer),
			},
			err: discoveryPackage.Error{Status: http.StatusNotFound, Body: gjson.Parse(`{
  "status": 404,
  "code": 1003,
...
func TestSeedExecution(t *testing.T) {
	tests := []struct {
		...
		// Working case
		{
			...
			details:        true,
			expectedOutput: "{\"audit\":[{\"stages\":[],\"status\":\"CREATED\",\"timestamp\":\"2025-09-05T20:09:22.543Z\"},{\"stages\":[],\"status\":\"RUNNING\",\"timestamp\":\"2025-09-05T20:09:26.621Z\"},{\"stages\":[\"BEFORE_HOOKS\"],\"status\":\"RUNNING\",\"timestamp\":\"2025-09-05T20:09:37.592Z\"},{\"stages\":[\"BEFORE_HOOKS\",\"INGEST\"],\"status\":\"RUNNING\",\"timestamp\":\"2025-09-05T20:13:26.602Z\"}],\"creationTimestamp\":\"2025-10-10T19:48:31Z\",\"dps\":1,\"id\":\"f85a5e19-8ed9-4f8c-9e2e-e1d5484612f3\",\"jobs\":{\"DONE\":5,\"RUNNING\":3},\"lastUpdatedTimestamp\":\"2025-10-10T19:48:31Z\",\"properties\":{\"stagingBucket\":\"testBucket\"},\"records\":{\"DONE\":4,\"PROCESSING\":4},\"scanType\":\"FULL\",\"stages\":[\"BEFORE_HOOKS\",\"INGEST\"],\"status\":\"RUNNING\",\"triggerType\":\"MANUAL\"}\n",
			printer:        JsonObjectPrinter(false),
			err:            nil,
		},
		{
		...
		// Error case
		{
			...
			client: new(mocks.FailingSeedExecutionGetterGetExecutionFails),
			summarizers: map[string]Summarizer{
				"records": new(mocks.WorkingRecordSummarizer),
				"jobs":    new(mocks.WorkingJobSummarizer),
			},
