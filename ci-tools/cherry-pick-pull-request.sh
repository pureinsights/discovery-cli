...
usage="Creates a pull request to merge the cherry pick into the target branch

Usage: $(basename "$0") -S SHA -s source -t target -i id [FLAGS]

Flags:
  -m        the message for the pull request
  -h        the current help message"

while getopts S:s:m:t:i:h flag
do
  case "${flag}" in
    S) sha=${OPTARG} ;;
    s) source=${OPTARG} ;;
...
if [[ -z ${sha} ]] || [[ -z ${source} ]] || [[ -z ${target} ]] || [[ -z ${id} ]]; then
  echo "${usage}" >&2
  exit 1
fi

branchName="bot/cherry-pick-${id}"
...
git checkout -b "${branchName}" --track "origin/${target}"
...
# Cherry pick the commit, but keep the current version
git cherry-pick "${sha}" --no-commit
git rm -f --ignore-unmatch ...
git reset HEAD -- semver.properties
